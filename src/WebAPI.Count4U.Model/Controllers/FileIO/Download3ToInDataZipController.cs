...
namespace Count4U.Service.Server.Controllers
{
    ...
    public class Download3ToInDataZipController : ControllerBase
    {
        private readonly ILogger<Download3ToInDataZipController> _logger;
        private readonly ICount4uContext _count4uContext;
		private readonly  ISettingsdb3Repository _settingsdb3Repository;
		public Download3ToInDataZipController(
             ILoggerFactory loggerFactory
            , ICount4uContext count4uContext
			, ISettingsdb3Repository settingsdb3Repository )
        {
            this._logger = loggerFactory.CreateLogger<Download3ToInDataZipController>();
        ...
        public async Task Post()
        {
             ...
             if (HttpContext.Request.Form.Files.Any()) 
            {
                foreach (var file in HttpContext.Request.Form.Files) 
                 {
                   	string path = _settingsdb3Repository.GetPathForInDataZip(file.FileName);
                    using (var stream = new FileStream(path, FileMode.Create))
                    {
                        await file.CopyToAsync(stream);
                    }
                        _settingsdb3Repository.UnZip(path);
                }
            }
        }
